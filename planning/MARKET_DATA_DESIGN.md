...
`threading.Lock` is used instead of `asyncio.Lock` because the Massive client's synchronous `get_snapshot_all()` runs in `asyncio.to_thread()`, which operates in a real OS thread — `asyncio.Lock` would not protect against that.
...
This push model decouples timing. The simulator ticks at 500ms, Massive polls at 15s, but SSE always reads from the cache at its own 500ms cadence. There is no need for the SSE layer to know which data source is active.
...
Constants only — no logic. Shared by the simulator (initial prices and GBM parameters) and potentially by the Massive client (fallback prices if the API hasn't responded yet).
...
Two classes:
- `GBMSimulator`: Pure math engine. Stateful — holds current prices, advances them one step.
...
`from massive import RESTClient` happens inside `start()`, not at module import time. Students who don't have a Massive API key don't need the package installed at all. The simulator path has zero external dependencies beyond `numpy`.
...
def create_stream_router(price_cache: PriceCache) -> APIRouter:
    ...
    async def stream_prices(request: Request) -> StreamingResponse:
        ...
        Streams all tracked ticker prices every ~500ms.
...
eventSource.onmessage = (event) => {
    ...
    // Flash green/red based on direction, update sparkline data
};
eventSource.onerror = () => {
...
async def add_to_watchlist(
    ...
    # Add to database, then tell the data source to start tracking it
...
async def remove_from_watchlist(
    ...
    # Only stop tracking if no open position (see section 11)
...
User/LLM → DELETE /api/watchlist/PYPL
  ...
  → If no position: await source.remove_ticker("PYPL")
      Both sources: remove from active list, remove from cache
  → Return {status: "ok"}
```

### Edge case: Ticker has an open position

If the user removes a ticker from the watchlist but still holds shares, the ticker must remain in the data source so portfolio valuation stays accurate:

```python
position = await db.get_position(ticker)
if position is None or position.quantity == 0:
    await source.remove_ticker(ticker)
# else: keep tracking it for portfolio valuation
```
...
class TestGBMSimulator:
    ...
    def test_cholesky_rebuilds_on_add(self):
        sim = GBMSimulator(tickers=["AAPL"])
        assert sim._cholesky is None  # Only 1 ticker, no matrix needed
        sim.add_ticker("GOOGL")
        assert sim._cholesky is not None
```

### 12.2 PriceCache Unit Tests
...
### 12.3 SimulatorDataSource Integration Tests
...
### 12.4 MassiveDataSource Unit Tests (Mocked)
...
If the database has no watchlist entries, `start()` receives `[]`. Both data sources handle this gracefully — the simulator produces no prices, the Massive poller skips its API call. When the user adds the first ticker, the source starts tracking it immediately.
...
If the API key is set but invalid, the first poll will fail with a 401. The poller logs the error and keeps retrying. The fix is to correct the API key and restart the container.

### Thread Safety Under Load

`PriceCache` uses `threading.Lock`. Under normal load (10 tickers, 2 updates/sec), lock contention is negligible. The critical section is tiny (dict lookup + assignment). This design supports future scaling without changes.
...
- Prices are always positive (exponential function guarantees this)
...
### Quick reference: downstream usage
